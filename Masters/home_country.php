...
session_start();
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		$country=trim($_POST['txtcla']);
		 ...
		 $query=mysql_query("SELECT * FROM tbl_country where country='$country' and country_id!='$country_id'") or die("Error: " . mysql_error());
		// exit;
		...
		<script>
		 ...
		 {
	  $sql_in="UPDATE tbl_country SET country='$country' where country_id ='$country_id'";
...
<title>Stores - Country Master - Country Home</title>
...
<?php
  ...
  $sql_arr_home=mysql_query("select * from tbl_country order by country desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from tbl_country order by country desc";
...
$pagination = "";
	...
	$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_country"),0); 
...
<td width="125" align="left" class="tblheading" valign="middle">&nbsp;Country Name</td>
...
//$srno=1;
	...
	{
		$resettargetquery=mysql_query("select * from tbl_partymaser where country='".$row['country']."'");
...
<td valign="middle" class="tbltext" align="center"><?php if($row['country']!="India"){?><a href="edit_country.php?country_id=<?php echo $row['country_id'];?>"><img src="../images/edit.png" border="0" /></a><?php } ?></td>

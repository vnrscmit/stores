...
$sql_tbl=mysql_query("select * from tblarrival where arr_role='".$logid."' and  arrival_type='Stocktransfer' and arrtrflag=0") or die(mysql_error());
while($row_tbl=mysql_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tblarrival where arr_role='".$logid."' and arrival_type='Stocktransfer' and arrtrflag=0";
	mysql_query($s_sub) or die(mysql_error());	
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Arrival from Stock Transfer </td>
	    </tr></table></td>
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from tblarrival where arrival_type='Stocktransfer' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $start, $limit") or die(mysql_error());
 $tot_arr_home=mysql_num_rows($sql_arr_home);

//$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_qctest where bflg=1   and qcflg=0"),0); 

$query = "select * from tblarrival where arrival_type='Stocktransfer' and arrtrflag=1 order by yearcode desc, arr_code desc";
...
$sql_arr_home=mysql_query("select * from tblarrival where arrival_type='Stocktransfer' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblarrival where arrival_type='Stocktransfer' and arrtrflag=1"),0); 
*/
    if($tot_arr_home >0) { 
?>

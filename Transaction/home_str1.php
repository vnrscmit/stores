<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='home_str1.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="820" height="25" class="Mainheading">&nbsp;Transaction - Issue- Stock Transfer </td>
	    </tr></table></td>
...
$sql_arr_home=mysql_query("select * from tblissue where issue_date between '$tdate' and '$pdate' and issue_type='stocktr' and issuetrflag=1 order by yearcode desc, iss_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblissue where issue_date between '$tdate' and '$pdate'  and issue_type='stocktr' and issuetrflag=1"),0); 
    if($tot_arr_home >0) { 
?>
...
<tr class="Light">
         ...
         <td width="12%" align="center" valign="middle" class="tblheading"><a href="add_issue_str_view.php?p_id=<?php echo $row_arr_home['issue_id'];?>"><?php echo "IS".$row_arr_home['iss_code']."/".$row_arr_home['yearcode']."/".$irole;?></a></td>
...
<tr >
<td valign="top" align="right"><a href="home_str.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:hand;" /></a></td>
</tr>
</table>

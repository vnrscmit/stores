...
$sql_tbl=mysql_query("select * from tbl_excess where esflg=0") or die(mysql_error());
while($row_tbl=mysql_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['tid'];	
	
	$s_sub="delete from tbl_excess_sub where esid='".$arrival_id."'";
	mysql_query($s_sub) or die(mysql_error());
	
}

	$s_sub="delete from tbl_excess where esflg=0";
	mysql_query($s_sub) or die(mysql_error());	
...
if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='add_shortage1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='add_shortage.php'</script>";
		}
	}
...
<title>Stores- Transaction -  Home Excess/Shortage</title>
...
{
winHandle=window.open('es_note_print.php?itmid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
...
<td align="center" width="130" valign="middle" class="tblbutn" style="cursor:hand;"><a href="add_excess.php" style="text-decoration:none; color:#FFFFFF">Add </a></td>
</tr>-->

<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_e1.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from  tbl_excess where  esflg=1 order by yearcode desc, code desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from  tbl_excess where  esflg=1 order by yearcode desc, code desc";
...
$sql_arr_home=mysql_query("select * from  tbl_excess where  esflg=1 order by yearcode desc, code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_excess where esflg=1"),0); */
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="12%" align="center" valign="middle" class="tblheading">Date</td>
              <td width="28%" align="center" valign="middle" class="tblheading">Classification</td>
			  <td width="33%" align="center" valign="middle" class="tblheading">Item</td>
...
<tr class="Light">
         ...
         <td width="13%" align="center" valign="middle" class="tblheading"><a href="add_exsh_view.php?p_id=<?php echo $row_arr_home['tid'];?>"><?php echo "ES".$row_arr_home['escode']."/".$row_arr_home['yearcode']."/".$lgnid;?></a></td>
		 <td width="12%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
		 
         <td width="28%" align="center" valign="middle" class="tblheading"><?php echo $noticia_class['classification'];?></td>
         <td width="33%" align="center" valign="middle" class="tblheading"><?php echo $noticia_item['stores_item'];?></td>
		  <td width="10%" align="center" valign="middle" class="tblheading"><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $row_arr_home['tid'];?>');">ESN</a></td>
         </tr>
<?php
}
else

...
<title>Stores -  Transazction - Discard Home</title>
...
{ 
  ...
  $sql_arr_home=mysql_query("select * from tbl_discard where tdate between '$tdate' and '$pdate' and ddflg=1 order by yearcode desc, dd_code desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from tbl_discard where tdate between '$tdate' and '$pdate' and ddflg=1 order by yearcode desc, dd_code desc";
...
$pagination = "";
	...
	{	
	 ...
	 $srno=($page-1)*$limit+1;
/*$sql_arr_home=mysql_query("select * from tbl_discard where tdate between '$tdate' and '$pdate' and ddflg=1 order by yearcode desc, dd_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_discard where tdate between '$tdate' and '$pdate' and ddflg=1"),0); */
?>
...
while($row_arr_home=mysql_fetch_array($sql_arr_home))
{
	...
	//$quer3=mysql_query("SELECT party_name FROM tbl_partymaser  where p_id='".$row_arr_home['party_id']."'"); 
...
?>
<?php echo $pagination?>
<table align="center" width="720" cellpadding="5" cellspacing="5" border="0" >
<tr >

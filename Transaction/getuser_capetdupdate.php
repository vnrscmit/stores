...
//frm_action=submit &txt11=By%20Hand &txt14= &txt15= &txt13= &txt= &txtid=26 &logid=opr1 &txtappli=Applicable &txt= &txtcode=26 &txtdate=30-06-2009 &txt12=Yes &txtparty= &txtaddress= &txtcla=78 &txtcontact=21321 &txtmode=Applicable &txt1=By%20Hand &txttname= &txtlrn= &txtvn= &txt13=--Select%20Mode-- &txtcname= &txtdc= &txtpname=mishra &txtclass=64 &txtitem=114 &txtuom=Number &txtrettyp=good &txtrettype=good &upsavl_1=0 &qtyavl_1=7664 &issueups_1= &issueqty_1= &balups_1= &balqty_1= &slocissue=111 &upsavl_2=2 &qtyavl_2=66 &issueups_2=1 &issueqty_2=10 &balups_2=1 &balqty_2=56 &srno=3 &chkbox=111 &srno1=2 &txtremarks=
...
if($txt12=="Yes")
{
 $sql_in1="insert into tbl_captive(code , tdate , party_id , contactno , tmode , tname , lrno , vno ,cname , docketno, pmode, pname, remarks,rettyp, yearcode, ccrole) values ('$code','$tdate','$f', '$contact','$txt1', '$i', '$j', '$k','$l', '$l','$m','$pname','txtremarks','$rettyp','$yearid_id','$logid')";
}
else
{
  $sql_in1="insert into tbl_captive(code , tdate , contactno , tmode , tname , lrno , vno ,cname , docketno, pmode, pname, party_name, address, address1, city, pin, state, remarks,rettyp, yearcode, ccrole) values ('$code','$tdate','$contact','$txt1', '$i', '$j', '$k','$l', '$l','$m','$pname','$party', '$txtaddress', '$txtaddress1', '$city', '$pin', '$state','$txtremarks','$rettyp','$yearid_id','$logid')";
}
 
if(mysql_query($sql_in1) or die(mysql_error()))
{
 $mainid=mysql_insert_id();
 $sql_in="insert into tbl_captivesub(id_in , classification_id , items_id , uom ,type ,remarks) values('$mainid', '$classid', '$itemid', '$txtuom', '$txtrettype', '$txtremarks')";
 if(mysql_query($sql_in) or die(mysql_error()))
{
$subid=mysql_insert_id(); $totups=0; $totqty=0;
...
$totqty=$totqty+$qty1;
$rowid=$p1_array[$num];
$sql_sub_sub="insert into tbl_captive_sloc (issue_type, issue_trid, isue_id, classification_id, item_id, whid, binid, subbin, qty_issue, ups_issue, qty_balance, ups_balance, issue_rowid, eid) values('captive','$mainid','$subid','$classid','$itemid','$whid','$binid','$subbinid','$qty1','$ups1','$balqty1','$balups1','$rowid','$rid')";

mysql_query($sql_sub_sub) or die(mysql_error());
}
$sql_sub_update="update tbl_captivesub set ups='$totups', qty='$totqty' where eid='$subid'";
...
$subid=$rid;
 ...
 $sql_in="update tbl_captivesub set id_in='$mainid', classification_id='$classid', items_id='$itemid', uom='$txtuom', type='$txtrettype', remarks='$txtremarks' where eid='$rid'";

if(mysql_query($sql_in) or die(mysql_error()))
{
$s_sub_sub="delete from tbl_captive_sloc where isue_id='".$subid."'";
...
<td valign="middle" class="tbltext" align="center"><img src="../images/edit.png" border="0"  onclick="editrec(<?php echo $row_eindent_sub['eid'];?>);" style="cursor:pointer" /></td>

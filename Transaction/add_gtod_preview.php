...
$sql_main="update tbl_gtod set  yearcode='$yearid_id', classification_id='$classid' , items_id='$itemid' , uom='$txtuom', remarks='$txtremarks', party_id='$party' where gid='".$pid."'";
...
if(isset($_POST['frm_action'])=='submit')
	...
	while($row_sub=mysql_fetch_array($sql_sub))
	{
		...
		$balqty1=$opqty1-$oqty;
				...
				$sql_sub_sub1="insert into tbl_stldg_good (yearcode,stlg_trtype, stlg_trsubtype, stlg_trpartyid, stlg_trid, stlg_trdate, stlg_trclassid, stlg_tritemid, stlg_whid, stlg_binid, stlg_subbinid, stlg_opups, stlg_opqty, stlg_trups, stlg_trqty, stlg_balups, stlg_balqty) values('$yearid_id','GD', 'GD', '$party', '$pid', '$trdate', '$classid1', '$itemid1', '$whid1', '$binid1', '$subbinid1', '$opups1', '$opqty1', '$oups', '$oqty', '$balups1', '$balqty1')";
				mysql_query($sql_sub_sub1) or die(mysql_error());
				
if($balqty1 == 0)
				{
				
$sql_issueg=mysql_query("select distinct stlg_whid, stlg_subbinid, stlg_binid from tbl_stldg_good where stlg_trclassid='".$classid1."' and stlg_tritemid!='".$itemid1."'") or die(mysql_error());
$cntg=0;
while($row_issueg=mysql_fetch_array($sql_issueg))
 { 
	$sql_issueg1=mysql_query("select max(stlg_id) from tbl_stldg_good where stlg_subbinid='".$row_issueg['stlg_subbinid']."' and stlg_binid='".$row_issueg['stlg_binid']."' and stlg_whid='".$row_issueg['stlg_whid']."' and stlg_tritemid!='".$itemid1."'") or die(mysql_error());
	...
	while($row_arrsub=mysql_fetch_array($sql_arrsub))
	{
		...
		$sql_issue2=mysql_query("select max(stld_id) from tbl_stldg_damage where stld_subbinid='".$subbinid."' and stld_binid='".$binid."' and stld_whid='".$whid."' and stld_tritemid='".$itemid."'") or die(mysql_error());
		...
		$t=mysql_num_rows($sql_issuetbl2);
		if($t > 0)
		{
		...
		$sql_sub_sub="insert into tbl_stldg_damage (yearcode,stld_trtype, stld_trsubtype, stld_trpartyid, stld_trid, stld_trdate, stld_trclassid, stld_tritemid, stld_whid, stld_binid, stld_subbinid, stld_opups, stld_opqty, stld_trups, stld_trqty, stld_balups, stld_balqty) values('$yearid_id','GD', 'GD', '$party', '$pid', '$trdate', '$classid', '$itemid', '$whid', '$binid', '$subbinid', '$opups', '$opqty', '$ups', '$qty', '$balups', '$balqty')";
	...
	$sql_sub12=mysql_query("SELECT sum(ups), sum(qty), classification_id, items_id FROM `tbl_gtod_sub` where gid='".$pid."'") or die(mysql_error());
	$row_sub12=mysql_fetch_array($sql_sub12);
...
$cntg=0;
	...
	$sql_code="SELECT MAX(gcode) FROM tbl_gtod ORDER BY gcode DESC";
	$res_code=mysql_query($sql_code)or die(mysql_error());
		...
		$r_chk=mysql_num_rows($s_chk);
		if($r_chk > 0)
		$sql_code1="SELECT MAX(ncode) FROM tbl_gtod where yearcode='$yearid_id' ORDER BY ncode DESC";
		else
		$sql_code1="SELECT MAX(ncode) FROM tbl_gtod ORDER BY ncode DESC";
	...
	$sql_main="update tbl_gtod set gdflg=1, gcode=$code, ncode=$ncode  where gid = '$pid'";
...
<title>Stores -Transaction G To D- Preview</title>

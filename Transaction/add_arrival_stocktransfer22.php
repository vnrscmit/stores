<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='add_arrival_stocktransfer22.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
...
<title>Stores-Transaction - Arrival from Stock Transfer</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="820" height="25" class="Mainheading">&nbsp;Transaction - Arrival from Stock Transfer </td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_arrival_stocktransfer.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onClick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
...
$sql_arr_home=mysql_query("select * from tblarrival where arrival_type='Stocktransfer' and arrtrflag=1") or die(mysql_error());
...
/*if(!isset($_GET['page'])) { 
  ...
  $sql_arr_home=mysql_query("select * from tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Stocktransfer' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $start, $limit") or die(mysql_error());
 $tot_arr_home=mysql_num_rows($sql_arr_home);

//$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_qctest where bflg=1   and qcflg=0"),0); 

$query = "select * from tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Stocktransfer' and arrtrflag=1 order by yearcode desc, arr_code desc";
...
$pagination = "";
	...
	{	
	 ...
	 $srno=($page-1)*$limit+1;
/*$sql_arr_home=mysql_query("select * from tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Stocktransfer' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblarrival where arrival_date between '$tdate' and '$pdate'  and arrival_type='Stocktransfer' and arrtrflag=1"),0); 
*/
    if($tot_arr_home >0) { 
?>
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="15%" align="center" valign="middle" class="tblheading">Transaction Id</td>
			  ...
			  <td width="21%" align="center" valign="middle" class="tblheading">STN No.</td>
...
<tr class="Light">
         ...
         <td width="15%" align="center" valign="middle" class="tblheading"><a href="add_arrival_stocktr_view.php?p_id=<?php echo $row_arr_home['arrival_id'];?>"><?php echo "AS".$row_arr_home['arr_code']."/".$row_arr_home['yearcode']."/".$lrole;?></a></td>
         <td width="42%" align="center" valign="middle" class="tblheading"><?php echo $row3['business_name'];?></td>
         <td width="21%" align="center" valign="middle" class="tblheading"><?php echo $row_arr_home['stnno']?></td>
         <td width="17%" align="center" valign="middle" class="tblheading"><a href="Javascript:void(0)" onClick="openslocpopprint('<?php echo $row_arr_home['arrival_id'];?>');">STRN</a></td>
</tr>
<?php
}
else

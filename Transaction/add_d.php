...
$sql_tbl=mysql_query("select * from tbl_dtog where dgflg=0") or die(mysql_error());
while($row_tbl=mysql_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['did'];	
	
	$s_sub="delete from tbl_dtog_sub where did='".$arrival_id."'";
	mysql_query($s_sub) or die(mysql_error());
	
}

	$s_sub="delete from tbl_dtog where dgflg=0";
	mysql_query($s_sub) or die(mysql_error());	
...
if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='add_d1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='add_d.php'</script>";
		}
	}
?>
...
<title>stores - Transction-Home Damage  To Good</title>
...
{
winHandle=window.open('dg_issue_note_print.php?itmid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="810" height="25">&nbsp;Transaction - Damage To Good </td>
	    </tr></table></td>
...
<td align="center" width="130" valign="middle" class="tblbutn" style="cursor:hand;"><a href="add_dtog.php" style="text-decoration:none; color:#FFFFFF">Add </a></td>
</tr>-->
<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_dtog.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from  tbl_dtog where dgflg=1 order by yearcode desc, dcode desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from  tbl_dtog where dgflg=1 order by yearcode desc, dcode desc";
...
$sql_arr_home=mysql_query("select * from  tbl_dtog where dgflg=1 order by yearcode desc, dcode desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);
*/
//$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_dtog where dgflg=1"),0); 
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Damage To Good</td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="14%" align="center" valign="middle" class="tblheading">Transaction Id</td>
			 <td width="10%" align="center" valign="middle" class="tblheading">Date</td>
              <td width="24%" align="center" valign="middle" class="tblheading">Classification</td>
			  <td width="37%" align="center" valign="middle" class="tblheading">Item</td>
...
while($row_arr_home=mysql_fetch_array($sql_arr_home))
{
	...
	$trdate=$row_arr_home['date'];
	$tryear=substr($trdate,0,4);
	$trmonth=substr($trdate,5,2);
	$trday=substr($trdate,8,2);
	$trdate=$trday."-".$trmonth."-".$tryear;	

$quer3=mysql_query("select classification_id, classification from tbl_classification where classification_id='".$row_arr_home['classification_id']."'") or die(mysql_error());
$noticia_class = mysql_fetch_array($quer3);

	$itemqry=mysql_query("select items_id, stores_item from tbl_stores where items_id='".$row_arr_home['items_id']."'") or die(mysql_error());
...
<tr class="Light">
         ...
         <td width="14%" align="center" valign="middle" class="tblheading"><a href="add_dtog_view.php?p_id=<?php echo $row_arr_home['did'];?>"><?php echo "DG".$row_arr_home['dcode']."/".$row_arr_home['yearcode']."/".$lgnid;?></a></td>
		 <td width="10%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
		 
         <td width="24%" align="center" valign="middle" class="tblheading"><?php echo $noticia_class['classification'];?></td>
         <td width="37%" align="center" valign="middle" class="tblheading"><?php echo $noticia_item['stores_item'];?></td>
		  <td width="11%" align="center" valign="middle" class="tblheading"><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $row_arr_home['did'];?>');">D2GN</a></td>
         </tr>
<?php
}
else

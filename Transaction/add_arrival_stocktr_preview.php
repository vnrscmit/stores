...
$sql_main="update tblarrival set yearcode='$yearid_id', stnno='$txtdcno', party_id='$txtcla', tmode='$txt11', trans_name='$txttname', trans_lorryrepno='$txtlrn', trans_vehno='$txtvn', trans_paymode='$txt14', courier_name='$txtcname', docket_no='$txtdc', pname_byhand='$txtpname', remarks='$remarks' where arrival_id = '$pid'";
	...
	while($row_arrsub=mysql_fetch_array($sql_arrsub))
	{
		...
		while($row_arrsub_sub=mysql_fetch_array($sql_arrsub_sub))
		{
			...
			if($row_arrsub_sub['qty_damage']==0 && $row_arrsub_sub['ups_damage']==0 )
			{
				...
				$sql_sub_sub="insert into tbl_stldg_good (yearcode, stlg_trtype, stlg_trsubtype, stlg_trid, stlg_trpartyid, stlg_trdate, stlg_trclassid, stlg_tritemid, stlg_whid, stlg_binid, stlg_subbinid, stlg_opups, stlg_opqty, stlg_trups, stlg_trqty, stlg_balups, stlg_balqty) values('$yearid_id','Arrival', 'Stocktransfer', '$pid', '$partyid', '$trdate', '$classid', '$itemid', '$whid', '$binid', '$subbinid', '$opups', '$opqty', '$ups', '$qty', '$balups', '$balqty')";
...
$cntg=0;
	...
	$a123456=mysql_query($sql_main) or die(mysql_error());
		...
		echo "<script>window.location='select_arrival_stocktrop.php?p_id=$pid'</script>";	
	}
...
<title>stores -Transaction - Arrival from Stock Transfer - Preview</title>
...
<td width="275"  align="left" valign="middle" class="tbltext">&nbsp;<?php echo "TAS".$row_tbl['arrival_code']."/".$yearid_id."/".$lgnid;
		?></td>

...
if(isset($_REQUEST['sdate']))
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='add_shortage1.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
...
<title>Stores- Transaction -Transaction - Excess/Shortage </title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="810" height="25" class="Mainheading">&nbsp;Transaction - Excess/Shortage </td>
	    </tr></table></td>
...
$sql_arr_home=mysql_query("select * from tbl_excess where tdate between '$tdate' and '$pdate' and esflg=1 order by yearcode desc, code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_excess where tdate between '$tdate' and '$pdate' and  esflg=1"),0); 
...
<tr >
<td valign="top" align="right"><a href="add_shortage.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:pointer;" /></a></td>
</tr>
</table>

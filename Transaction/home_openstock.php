...
$sql_tbl=mysql_query("select * from tblarrival where arr_role='".$logid."' and  arrival_type='OP' and arrtrflag=0") or die(mysql_error());
while($row_tbl=mysql_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tblarrival where arr_role='".$logid."' and arrival_type='OP' and arrtrflag=0";
	mysql_query($s_sub) or die(mysql_error());	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_openstock1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='home_openstock.php'</script>";
		}
	}
...
<title>stores - Transaction-Home opening Stock</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="810" height="25">&nbsp;Transaction - Opening Stock </td>
	    </tr></table></td>
...
<td align="center" width="130" valign="middle" class="tblbutn" style="cursor:hand;"><a href="add_stock.php" style="text-decoration:none; color:#FFFFFF">Add </a></td>
</tr>-->
<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_stock.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
...
$sql_arr_home=mysql_query("select * from tblarrival where arr_role='".$logid."' and arrival_type='OP' and arrtrflag=1") or die(mysql_error());
...
<tr>
<td width="30">	 </td><td><table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from tblarrival where arr_role='".$logid."' and arrival_type='OP' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $start, $limit") or die(mysql_error());
 $tot_arr_home=mysql_num_rows($sql_arr_home);

//$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_qctest where bflg=1   and qcflg=0"),0); 

$query = "select * from tblarrival where arr_role='".$logid."' and arrival_type='OP' and arrtrflag=1 order by yearcode desc, arr_code desc";
...
$pagination = "";
	...
	{	
		...
		//previous button
		if ($page > 1) 
			$pagination.= " <a href=\"$targetpage?page=$prev&logid=$logid\">« previous </a> ";
		else
		...
		{	
			for ($counter = 1; $counter <= $lastpage; $counter++)
			{
				if ($counter == $page)
					...
					$pagination.= " <a href=\"$targetpage?page=$counter&logid=$logid\"> $counter </a> ";					
			}
		}
		elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
		{
			...
			if($page < 1 + ($adjacents * 2))		
			{
				...
				$pagination.= " ... ";
				$pagination.= " <a href=\"$targetpage?page=$lpm1&logid=$logid\"> $lpm1 </a> ";
				$pagination.= " <a href=\"$targetpage?page=$lastpage&logid=$logid\"> $lastpage </a> ";		
			}
			//in middle; hide some front and some back
			elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
			{
				$pagination.= " <a href=\"$targetpage?page=1&logid=$logid\"> 1 </a> ";
				$pagination.= " <a href=\"$targetpage?page=2&logid=$logid\"> 2 </a> ";
				$pagination.= " ... ";
		...
		//next button
		if ($page < $counter - 1) 
			$pagination.= " <a href=\"$targetpage?page=$next&logid=$logid\"> next »</a> ";
		else
...
$sql_arr_home=mysql_query("select * from tblarrival where arr_role='".$logid."' and arrival_type='OP' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblarrival where arr_role='".$logid."' and arrival_type='OP' and arrtrflag=1"),0); */
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="18%" align="center" valign="middle" class="tblheading">Transaction Id</td>
			 <td width="12%" align="center" valign="middle" class="tblheading">Date</td>
              <td width="26%" align="center" valign="middle" class="tblheading">Classification</td>
			  <td width="39%" align="center" valign="middle" class="tblheading">Item</td>
			   </tr>
<?php
//$srno=1;
if($tot_arr_home > 0)
...
$sql_tbl_sub=mysql_query("select * from tblarrival_sub where arrival_id='".$row_arr_home['arrival_id']."'") or die(mysql_error());
$row_tbl_sub=mysql_fetch_array($sql_tbl_sub);

$quer3=mysql_query("select classification_id, classification from tbl_classification where classification_id='".$row_tbl_sub['classification_id']."'") or die(mysql_error());
$noticia_class=mysql_fetch_array($quer3);

$itemqry1=mysql_query("select * from tbl_stores where items_id='".$row_tbl_sub['item_id']."'") or die(mysql_error());
...
<tr class="Light">
         ...
         <td width="18%" align="center" valign="middle" class="tblheading"><a href="home_openstock_view.php?p_id=<?php echo $row_arr_home['arrival_id'];?>"><?php echo "OP".$row_arr_home['arr_code']."/".$row_arr_home['yearcode']."/".$lgnid?></a></td>
		 <td width="12%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
		 
         <td width="26%" align="center" valign="middle" class="tblheading"><?php echo $noticia_class['classification'];?></td>
         <td width="39%" align="center" valign="middle" class="tblheading"><?php echo $row_itm['stores_item'];?></td>
		  </tr>
<?php
}
else

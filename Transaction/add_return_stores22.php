<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='add_return_stores22.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
...
/*if(!isset($_GET['page'])) { 
  ...
  $sql_arr_home=mysql_query("select * from tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Internalreturn' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $start, $limit") or die(mysql_error());
 $tot_arr_home=mysql_num_rows($sql_arr_home);

//$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_qctest where bflg=1   and qcflg=0"),0); 

$query = "select * from tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Internalreturn' and arrtrflag=1 order by yearcode desc, arr_code desc";
...
$pagination = "";
	...
	{	
	 ...
	 $srno=($page-1)*$limit+1;
/*$sql_arr_home=mysql_query("select * from tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Internalreturn' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Internalreturn' and arrtrflag=1"),0); */
...
<tr >
<td valign="top" align="right"><a href="add_return_stores2.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:hand;" /></a></td>
</tr>
</table>

<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_transction_issue_einternal1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='home_transaction_issue_einternal.php'</script>";
		}
	}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="810" height="25">&nbsp;Issue Types - Internal Issue - e-Indent</td>
	    </tr></table></td>
...
<?php
	$adjacents = 3;
$start = 1;	
	$query =  mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblissue where issue_type='eindent' and issuetrflag=1"),0); 
	//$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $query;
	
	/* Setup vars for query. */
	$targetpage = "home_transction_issue_einternal.php"; 	//your file name  (the name of this file)
	...
	if($page) 
		...
		$start = 0;	
		//echo $start;							//if no page var is given, set start to 0
	
	/* Get data. */
	$sql = "select * from tblissue where issue_type='eindent' and issuetrflag=1 order by yearcode desc, iss_code desc LIMIT $start, $limit";
...
while($row_arr_home=mysql_fetch_array($sql_arr_home))
{
	...
	$sql_ier1=mysql_query("select max(tid) from tbl_ieindent where  code='".$row_arr_home['dcrefno']."' and yearcode='".$row_arr_home['yearcode']."'") or die(mysql_error());
	$row_ier1=mysql_fetch_array($sql_ier1);
	$sql_ier=mysql_query("select * from tbl_ieindent where tid='".$row_ier1[0]."' and code='".$row_arr_home['dcrefno']."' and yearcode='".$row_arr_home['yearcode']."'") or die(mysql_error());
...
<tr >
<td valign="top" align="right"><a href="select_transction_issue_internal.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:pointer;" /></a></td>
</tr>
</table>
</td><td width="30"></td>
</tr>

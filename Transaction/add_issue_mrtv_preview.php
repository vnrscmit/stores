...
$sql_main="update tblissue set yearcode='$yearid_id', dcrefno='$txtpdc', party_id='$txtcla', tmode='$txt11', trans_name='$txttname', trans_lorryrepno='$txtlrn', trans_vehno='$txtvn', trans_paymode='$txt14', courier_name='$txtcname', docket_no='$txtdc', pname_byhand='$txtpname', remarks='$remarks', rettyp='$rettyp' where issue_id = '$pid'";
 ...
 { 
	...
	if($totnog > 0)
	{
	  $cntg++;
			...
			$dcups=0;
			$dcqty=0;
			$goodups=$row_arrsub['ups_indent'];
			$goodqty=$row_arrsub['qty_indent'];
			$damageups=0;
			$damageqty=0;
			$exqty=0;
			$shqty=0;
			...
			$balanceups=$openups+$goodups;
			$balaceqty=$openqty+$goodqty;
			}
			else
			{
			$balanceups=$goodups;
...
$cntg=0;
	...
	if($r_chk > 0)
	$sql_code="SELECT MAX(iss_code) FROM tblissue where yearcode='$yearid_id' and issue_type='MReturnV' ORDER BY issue_code DESC";
	else
	$sql_code="SELECT MAX(iss_code) FROM tblissue where issue_type='MReturnV' ORDER BY issue_code DESC";
	...
	$res_code=mysql_query($sql_code)or die(mysql_error());
		...
		if($r_chk > 0)
		$sql_code1="SELECT MAX(ncode) FROM tblissue where yearcode='$yearid_id' and issue_type='MReturnV' ORDER BY ncode DESC";
		else
		$sql_code1="SELECT MAX(ncode) FROM tblissue where yearcode='$yearid_id' and  issue_type='MReturnV' ORDER BY ncode DESC";
...
<title>stores -Transction Issue - Material Return to Vendor</title>
...
<td width="204"  align="left" valign="middle" class="tbltext">&nbsp;<?php echo "TIM".$row['issue_code']."/".$yearid_id."/".$logid;?></td>

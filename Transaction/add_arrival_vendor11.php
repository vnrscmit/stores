<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='add_arrival_vendor11.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="820" height="25" class="Mainheading">&nbsp;Transaction - Arrival From Vendor </td>
	    </tr></table></td>
...
/*if(!isset($_GET['page'])) { 
  ...
  $sql_arr_home=mysql_query("select * from tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Vendor' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Vendor' and arrtrflag=1 order by yearcode desc, arr_code";
...
$pagination = "";
	...
	{	
	 ...
	 $srno=($page-1)*$limit+1;
/*$sql_arr_home=mysql_query("select * from tblarrival where arrival_date between '$tdate' and '$pdate' and arrival_type='Vendor' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblarrival where arrival_date between '$tdate' and '$pdate'  and arrival_type='Vendor' and arrtrflag=1"),0); 
*/
    if($tot_arr_home >0) { 
?>
...
<tr >
<td valign="top" align="right"><a href="add_arrival_vendor1.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:hand;" /></a></td>
</tr>
</table>

<?php
	...
	$row_yr=mysql_fetch_array($sql_yr);
		...
		if($rg1>0)$lstdtg=strtotime($last_tdateg);else $lstdtg=strtotime(date("Y-m-d"));
		...
		if($rd1>0)$lstdtd=strtotime($last_tdated);else $lstdtd=strtotime(date("Y-m-d"));
...
$sql_tbl=mysql_query("select * from tblarrival where arr_role='".$logid."' and  arrival_type='Vendor' and arrtrflag=0") or die(mysql_error());
while($row_tbl=mysql_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tblarrival where arr_role='".$logid."' and arrival_type='Vendor' and arrtrflag=0";
	mysql_query($s_sub) or die(mysql_error());	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='add_arrival_vendor11.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='add_arrival_vendor1.php'</script>";
		}
	}
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Arrival From Vendor </td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_arrival_vendor.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from tblarrival where arrival_type='Vendor' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from tblarrival where arrival_type='Vendor' and arrtrflag=1 order by yearcode desc, arr_code desc";
...
$sql_arr_home=mysql_query("select * from tblarrival where arrival_type='Vendor' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblarrival where arrival_type='Vendor' and arrtrflag=1"),0); 
*/
    if($tot_arr_home >0) { 
?>
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Arrival From Vendor </td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="15%" align="center" valign="middle" class="tblheading">Date</td>
              <td width="42%" align="center" valign="middle" class="tblheading">Vendor</td>
			  <td width="21%" align="center" valign="middle" class="tblheading">DC/Inv No.</td>
...
<tr class="Light">
         ...
         <td width="15%" align="center" valign="middle" class="tblheading"><a href="add_arrival_vendor_view.php?p_id=<?php echo $row_arr_home['arrival_id'];?>"><?php echo "AV".$row_arr_home['arr_code']."/".$row_arr_home['yearcode']."/".$lrole;?></a></td>
		 <td width="5%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
         <td width="42%" align="center" valign="middle" class="tblheading"><?php echo $row3['business_name'];?></td>
         <td width="21%" align="center" valign="middle" class="tblheading"><?php echo $row_arr_home['dcno']?></td>
         <td width="17%" align="center" valign="middle" class="tblheading"><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $row_arr_home['arrival_id'];?>');">GRN</a></td>
</tr>
<?php
}
else

...
//frm_action=submit &txt11=By%20Hand &txt14= &txtrettype=good &txtid=1 &logid=opr1 &txtid1=IM1 &date=29-06-2009 &txtcla=77 &txtpdc=132564 &txt1=By%20Hand &txttname= &txtlrn= &txtvn= &txt13=--Select%20Mode-- &txtcname= &txtdc= &txtpname=Mishraji &txtclass=64 &txtitem=114 &txtuom=Number &txtups=1 &txtqty=10 &txtrettyp=good &upsavl_1=0 &qtyavl_1=7689 &issueups_1= &issueqty_1= &balups_1= &balqty_1= &slocissue=91 &upsavl_2=8 &qtyavl_2=80 &issueups_2=1 &issueqty_2=10 &balups_2=7 &balqty_2=70 &srno=3 &chkbox=91 &srno1=2 &txtremarks=
...
if($trid == 0)
{
$sql_main="insert into tblissue (issue_type, issue_code, issue_date, dcrefno, rettype, party_id, tmode, trans_name, trans_lorryrepno, trans_vehno, trans_paymode, courier_name, docket_no, pname_byhand, issue_role ,rettyp) values('MReturnV','$code','$tdate','$txtpdc','$txtrettype','$f','$txt1','$i','$j','$k','$txt13','$l','$m','$pname','$logid','$rettyp')";

if(mysql_query($sql_main) or die(mysql_error()))
{
$mainid=mysql_insert_id();

$sql_sub="insert into tblissue_sub (issue_id, classification_id, item_id, uom, rettype) values('$mainid','$classid','$itemid','$txtuom','$txtrettype')";
...
for($num=0; $num<$numrec; $num++)
{
	...
	$p1_array1[$num];
		...
		$totqty=$totqty+$qty1;
		$rowid=$p1_array[$num];
		$sql_sub_sub="insert into tblissue_sloc (issue_type, issue_tr_id, issue_id, classification_id, item_id, whid, binid, subbin, qty_issue, ups_issue, qty_balance, ups_balance, issue_rowid, eid) values('MReturnV','$mainid','$subid','$classid','$itemid','$whid','$binid','$subbinid','$qty1','$ups1','$balqty1','$balups1','$rowid','$rid')";
		
		mysql_query($sql_sub_sub) or die(mysql_error());
}

$sql_sub_update="update tblissue_sub set ups_indent='$totups', qty_indent='$totqty', rettype='$txtrettype' where issuesub_id='$subid'";
...
$sql_sub="update tblissue_sub set classification_id='$classid', item_id='$itemid', uom='$txtuom', rettype='$txtrettype' where issuesub_id='".$edtrowid."'";

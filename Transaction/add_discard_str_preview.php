...
$a123456=mysql_query($sql_main) or die(mysql_error());
	...
	$sql_arr=mysql_query("select * from tbl_discard where tid='".$pid."'") or die(mysql_error());
	while($row_arr=mysql_fetch_array($sql_arr))
	{
	$partyid=$row_arr['party_name'];
	$trdate=$row_arr['tdate'];
	
	$sql_arrsub=mysql_query("select * from tbl_discard_sub where did_s='".$pid."'") or die(mysql_error());
	while($row_arrsub=mysql_fetch_array($sql_arrsub))
	{
		$classid=$row_arrsub['calssification_id'];
		$itemid=$row_arrsub['items_id'];
		
		$sql_arrsub_sub=mysql_query("select * from tbl_discard_sloc where discard_trid='".$pid."' and discard_id='".$row_arrsub['did']."'") or die(mysql_error());
...
$sql_issued=mysql_query("select distinct stld_whid, stld_subbinid, stld_binid from tbl_stldg_damage where stld_trclassid='".$classid."' and stld_tritemid!='".$itemid."'") or die(mysql_error());
$totups=0; $totqtyd=0; $cntd=0;
while($row_issued=mysql_fetch_array($sql_issued))
 { 
	$sql_issued1=mysql_query("select max(stld_id) from tbl_stldg_damage where stld_subbinid='".$row_issued['stld_subbinid']."' and stld_binid='".$row_issued['stld_binid']."' and stld_whid='".$row_issued['stld_whid']."'") or die(mysql_error());
...
<title>stores -Transction -Material Discard</title>

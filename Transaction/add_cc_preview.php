...
if($txt12=="No")
{	
	$sql_main="update tbl_captive set contactno='$contact', tmode='$txt1', tname='$txttname', lrno='$txtlrn', vno='$txtvn', cname='$txtcname', docketno='$txtdc', pmode='$txt14', pname='$txtpname', party_name='$party', address='$txtaddress', address1='$txtaddress1', city='$city', pin='$pin', state='$state', remarks='$remarks', rettyp='$rettyp' where tid='$pid'";
}
else
{	
	 $sql_main="update tbl_captive set party_id='$txtcla', contactno='$contact', tmode='$txt1', tname='$txttname', lrno='$txtlrn', vno='$txtvn', cname='$txtcname', docketno='$txtdc', pmode='$txt14', pname='$txtpname', remarks='$remarks', rettyp='$rettyp' where tid='$pid'";
}
...
$a123456=mysql_query($sql_main) or die(mysql_error());
	...
	$sql_arr=mysql_query("select * from tbl_captive where tid='".$pid."'") or die(mysql_error());
	...
	$sql_arrsub=mysql_query("select * from tbl_captivesub where id_in='".$pid."'") or die(mysql_error());
	while($row_arrsub=mysql_fetch_array($sql_arrsub))
	{
		$classid=$row_arrsub['classification_id'];
		$itemid=$row_arrsub['items_id'];
		
		$sql_arrsub_sub=mysql_query("select * from tbl_captive_sloc where issue_trid='".$pid."' and isue_id='".$row_arrsub['eid']."'") or die(mysql_error());
		while($row_arrsub_sub=mysql_fetch_array($sql_arrsub_sub))
		{
			...
			$qty=$row_arrsub_sub['qty_issue'];
				...
				$sql_sub_sub="insert into tbl_stldg_good (yearcode,stlg_trtype, stlg_trsubtype, stlg_trid, stlg_trpartyid, stlg_trdate, stlg_trclassid, stlg_tritemid, stlg_whid, stlg_binid, stlg_subbinid, stlg_opups, stlg_opqty, stlg_trups, stlg_trqty, stlg_balups, stlg_balqty) values('$yearid_id','CC', 'CC', '$pid', '$partyid', '$trdate', '$classid', '$itemid', '$whid', '$binid', '$subbinid', '$opups', '$opqty', '$ups', '$qty', '$balups', '$balqty')";
...
while($row_issueg=mysql_fetch_array($sql_issueg))
 { 
	$sql_issueg1=mysql_query("select max(stlg_id) from tbl_stldg_good where stlg_subbinid='".$row_issueg['stlg_subbinid']."' and stlg_binid='".$row_issueg['stlg_binid']."' and stlg_whid='".$row_issueg['stlg_whid']."' and stlg_tritemid!='".$itemid."'") or die(mysql_error());
...
$cntg=0;
	...
	if($r_chk > 0)
	$sql_code="SELECT MAX(cc_code) FROM tbl_captive where yearcode='$yearid_id' ORDER BY cc_code DESC";
	else
	$sql_code="SELECT MAX(cc_code) FROM   tbl_captive  ORDER BY cc_code DESC";
	$res_code=mysql_query($sql_code)or die(mysql_error());
		...
		$r_chk=mysql_num_rows($s_chk);
		if($r_chk > 0)
		$sql_code1="SELECT MAX(ncode) FROM tbl_captive where yearcode='$yearid_id' ORDER BY ncode DESC";
		else
	...
	$sql_main="update tbl_captive set ccflg=1, cc_code=$code, ncode='$ncode'  where tid = '$pid'";

	$a123456=mysql_query($sql_main) or die(mysql_error());
	
	$s_chk=mysql_query("SELECT * FROM tbl_gate where yearcode='$yearid_id'") or die (mysql_error());
	$r_chk=mysql_num_rows($s_chk);
	if($r_chk > 0)
	$sql_code2="SELECT MAX(gpcode) FROM tbl_gate where yearcode='$yearid_id' ORDER BY gpcode DESC";
	else
	$sql_code2="SELECT MAX(gpcode) FROM tbl_gate ORDER BY gpcode DESC";
	$rescode2=mysql_query($sql_code2)or die(mysql_error());
		
		if(mysql_num_rows($rescode2) > 0) 
			{
				$row_code2=mysql_fetch_row($rescode2);
				$t_code2=$row_code2['0'];
				$gpcode=$t_code2+1;
			}	
		else 
...
mysql_query($sql_gp) or die (mysql_error());
			...
			echo "<script>window.location='select_cc_op.php?p_id=$pid'</script>";	
}
/*$a="CC";
	$sql_code="SELECT MAX(cc_code) FROM tbl_captive ORDER BY cc_code DESC";
	$res_code=mysql_query($sql_code)or die(mysql_error());
...
winHandle=window.open('issue_cc_print.php?&pid='+pid,'WelCome','top=170,left=180,width=820,height=350,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <td width="940" class="Mainheading" height="25"><table width="940" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
        <tr >
          <td width="813" height="25" class="Mainheading">&nbsp;Transction Issue - Captive Consumption</td>
        </tr>
	    </table></td>
	  </tr>
	  </table></td></tr>
...
<td align="center" class="smalltbltext" valign="middle"><?php echo $noticia_class['classification'];?></td>
              ...
              <td width="4%" align="center" valign="middle" class="smalltbltext"><?php echo $balqty;?></td><input type="hidden" name="rid" value="<?php echo $row_eindent_sub['eid'];?>" />
</tr>
<?php
	}
	else

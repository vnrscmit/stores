...
$sql_tblsub=mysql_query("select * from tbl_discard_sub where did='".$rid."'") or die(mysql_error());
//echo $t=mysql_num_rows($sql_tblsub);
$row_tblsub=mysql_fetch_array($sql_tblsub);

$trid=$row_issuesub['did_s'];
$itmdchk="";
$sql_eindent_sub=mysql_query("select * from tbl_discard_sub where did_s='".$trid."'") or die(mysql_error());
...
$class_sql=mysql_query("select classification_id, classification from tbl_classification where classification_id='".$row_tblsub['calssification_id']."'") or die(mysql_error());
$row_class = mysql_fetch_array($class_sql);

$item_sql=mysql_query("select items_id, stores_item,uom from tbl_stores where items_id='".$row_tblsub['items_id']."'") or die(mysql_error());
...
<option value="" >--Select Classification--</option>
	<?php while($noticia_class = mysql_fetch_array($qry)) { ?>
		<option <?php if($noticia_class['classification_id']==$row_tblsub['calssification_id']){ echo "Selected"; } ?> value="<?php echo $noticia_class['classification_id'];?>" />   
		<?php echo $noticia_class['classification'];?>
		<?php } ?>
	</select>&nbsp;<font color="#FF0000">*</font>&nbsp;</td>
         </tr>
		<?php 
$itemqry=mysql_query("select items_id, stores_item from tbl_stores where classification_id='".$row_tblsub['calssification_id']."' and actstatus='Active'") or die(mysql_error());
?> 
		<tr class="Light" height="25" id="vitem">
...
<option value="" >--Select Item--</option>
	<?php while($noticia_item = mysql_fetch_array($itemqry)) { ?>
		<option <?php if($noticia_item['items_id']==$row_tblsub['items_id']){ echo "Selected"; } ?> value="<?php echo $noticia_item['items_id'];?>" />   
...
<td width="169"  align="left" valign="middle" class="tbltext" id="uom">&nbsp;<input name="txtuom" type="text" size="10" class="tbltext" tabindex="0" readonly="true" style="background-color:#CCCCCC"  value="<?php echo $row_item['uom'];?>" /></td>
         </tr><input type="hidden" name="itmdchk" value="<?php echo $itmdchk;?>" />	
 <tr class="Dark" height="30" >
<td align="right" valign="middle" class="tblheading">UPS&nbsp;</td>
<td width="349" align="left" valign="middle" class="tbltext" >&nbsp;<input name="txtups" type="text" size="10" class="tbltext" tabindex="0" maxlength="5" onchange="piupschk();" onkeypress="return isNumberKey(event)" value="<?php echo $row_tblsub['ups'];?>" />&nbsp;<font color="#FF0000">*</font>&nbsp;</td>
		
<td width="68" align="right"  valign="middle" class="tblheading" >Quantity&nbsp;</td>
<td width="169"  align="left" valign="middle" class="tbltext" >&nbsp;<input name="txtqty" type="text" size="10" class="tbltext" tabindex="0" maxlength="7" onchange="piqtychk(this.value);" onkeypress="return isNumberKey(event)" value="<?php echo $row_tblsub['qty'];?>" />&nbsp;<font color="#FF0000">*</font>&nbsp;</td>
         </tr>	
...
<table align="center" border="1" width="750" cellspacing="0" cellpadding="0" bordercolor="#4ea1e1" style="border-collapse:collapse" > <tr class="tblsubtitle" height="20">
...
$sql_tbl_sub=mysql_query("select * from tbl_discard_sloc where discard_id=$rid") or die(mysql_error());
$tot_tblissue=mysql_num_rows($sql_tbl_sub);

while($row_tbl_sub=mysql_fetch_array($sql_tbl_sub))
{

$sql_issuetbl=mysql_query("select * from tbl_stldg_damage where stld_id='".$row_tbl_sub['discard_rowid']."'") or die(mysql_error()); 
...
$sql_issue=mysql_query("select distinct stld_whid, stld_subbinid, stld_binid from tbl_stldg_damage where stld_trclassid='".$row_tblsub['calssification_id']."' and stld_tritemid='".$row_tblsub['items_id']."'") or die(mysql_error());

while($row_issue=mysql_fetch_array($sql_issue))
 { 
 
$sql_issue1=mysql_query("select max(stld_id) from tbl_stldg_damage where stld_subbinid='".$row_issue['stld_subbinid']."' and stld_binid='".$row_issue['stld_binid']."' and stld_whid='".$row_issue['stld_whid']."' and stld_tritemid='".$row_tblsub['items_id']."'") or die(mysql_error());
...
$sql_issuetbl=mysql_query("select * from tbl_stldg_damage where stld_id='".$row_issue1[0]."'  and stld_balqty > 0") or die(mysql_error()); 
 ...
 { 
$sql_tbl_sub=mysql_query("select * from tbl_discard_sloc where discard_rowid='".$row_issuetbl['stld_id']."' and discard_id='".$rid."'") or die(mysql_error());
$tot_tblissue=mysql_num_rows($sql_tbl_sub);

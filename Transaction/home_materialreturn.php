...
session_start();
		...
		if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='home_materialreturn1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='home_materialreturn.php'</script>";
		}
	}
?>
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from tblissue where issue_type='MReturnV' and issuetrflag=1 order by yearcode desc, iss_code desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from tblissue where issue_type='MReturnV' and issuetrflag=1 order by yearcode desc, iss_code desc";
...
$sql_arr_home=mysql_query("select * from tblissue where issue_type='MReturnV' and issuetrflag=1 order by yearcode desc, iss_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblissue where issue_type='MReturnV' and issuetrflag=1"),0); 
*/
    if($tot_arr_home >0) { 
?>
...
<tr >
<td valign="top" align="right"><a href="issue_home.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:hand;" /></a></td>
</tr>
</table>
</td><td width="30"></td>
</tr>

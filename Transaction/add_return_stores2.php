<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='add_return_stores22.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='add_return_stores2.php'</script>";
		}
	}
...
<title>stores - Tranasction- Home internal Return</title>
...
{
//var itm=document.frmaddDepartment.txtitem.value;
winHandle=window.open('arr_ternald_print.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); 
}
}
...
$sql_arr_home=mysql_query("select * from tblarrival where arr_role='".$logid."' and arrival_type='Internalreturn' and arrtrflag=1") or die(mysql_error());
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Internal Return </td>
	    </tr></table></td>
	  <td width="100" height="25" align="right" class="submenufont" >
	  <table border="3" align="right" bordercolordark="#5b7e1b" cellspacing="0" cellpadding="0" width="111" style="border-collapse:collapse;">

<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_return_stores_st.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onClick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
 ...
 $targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from tblarrival where arrival_type='Internalreturn' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $start, $limit") or die(mysql_error());
 $tot_arr_home=mysql_num_rows($sql_arr_home);

//$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_qctest where bflg=1   and qcflg=0"),0); 

$query = "select * from tblarrival where arrival_type='Internalreturn' and arrtrflag=1 order by yearcode desc, arr_code desc";
...
$sql_arr_home=mysql_query("select * from tblarrival where arrival_type='Internalreturn' and arrtrflag=1 order by yearcode desc, arr_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblarrival where arrival_type='Internalreturn' and arrtrflag=1"),0); 
*/
    if($tot_arr_home >0) { 
?>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="15%" align="center" valign="middle" class="tblheading">Transaction Id</td>
              <td width="42%" align="center" valign="middle" class="tblheading">Return Stage</td>
			  <td width="21%" align="center" valign="middle" class="tblheading">Return by ID</td>
...
if($row_arr_home['type']=="Good")$flg=1; 
elseif($row_arr_home['type']=="Damage")$flg=2; 
else $flg=0;


/*$quer11=mysql_query("SELECT id , stage FROM tbl_roles ")or die(mysql_error());  
$rowp=mysql_fetch_array($quer11);
		
$quer1=mysql_query("SELECT id ,login FROM tbl_roles ")or die(mysql_error());  
$row3=mysql_fetch_array($quer1);
	*/
if($srno%2!=0)
{
?>			  
<tr class="Light">
         ...
         <td width="15%" align="center" valign="middle" class="tblheading"><a href="add_returnd_stores_view.php?p_id=<?php echo $row_arr_home['arrival_id'];?>"><?php echo "AI".$row_arr_home['arr_code']."/".$row_arr_home['yearcode']."/".$lrole;?></a></td>
         <td width="42%" align="center" valign="middle" class="tblheading"><?php echo $row_arr_home['stageret']?></td>
         <td width="21%" align="center" valign="middle" class="tblheading"><?php echo $row_arr_home['retid']?></td>
         <td width="17%" align="center" valign="middle" class="tblheading"><?php if($flg==1) { ?><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $row_arr_home['arrival_id'];?>');">IMRN-O</a><?php } else if($flg==2){ ?><a href="Javascript:void(0)" onclick="openslocpopprint1('<?php echo $row_arr_home['arrival_id'];?>');">IMRN-P</a><?php } else {?><?php } ?></td>
</tr>
<?php
}
else
...
<tr >
<td valign="top" align="right"><a href="arrival_home.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:hand;" /></a></td>
</tr>
</table>

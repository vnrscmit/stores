...
$sql_tbl=mysql_query("select * from tbl_discard where ddrole='".$logid."' and ddflg=0") or die(mysql_error());
while($row_tbl=mysql_fetch_array($sql_tbl))
{
	...
	$s_sub="delete from tbl_discard where ddrole='".$logid."' and ddflg=0";
	mysql_query($s_sub) or die(mysql_error());	
...
<title>Stores -  Transaction - Discard Home</title>
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from tbl_discard where  ddflg=1 order by yearcode desc, dd_code desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from tbl_discard where  ddflg=1 order by yearcode desc, dd_code desc";
...
$sql_arr_home=mysql_query("select * from tbl_discard where  ddflg=1 order by yearcode desc, dd_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_discard where  ddflg=1"),0); 
*/
   ?>

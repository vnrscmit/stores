<?php
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate!="" && $edate!="")
		{
		echo "<script>window.location='home_transction_issue_einternal1.php?sdate=$sdate&edate=$edate'</script>";
		}
		else
		 {?>
		 <script>
...
<title>Stores - Tranasction -Home e-Indent</title>
...
function openslocpopprint(itm,tid)
{
winHandle=window.open('eindent_note_print.php?itmid='+itm+'&tid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="810" height="25" class="Mainheading">&nbsp;Issue Types - Internal Issue - e-Indent</td>
	    </tr></table></td>
...
-->
<tr height="15" class="tbltitle" >
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_issue_indents1.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onClick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
</table></td>
...
$edate1=$edate;
	...
	$tdate1=split("-",$sdate);
	$sdate=$tdate1[2]."-".$tdate1[1]."-".$tdate1[0];	
	...
	$query =  mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tblissue where issue_type='eindent' and issue_date>='$sdate' and issue_date<='$edate' and issuetrflag=1"),0); 
	//$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $query;
	
	/* Setup vars for query. */
	$targetpage = "home_transction_issue_einternal1.php"; 	//your file name  (the name of this file)
	...
	/* Get data. */
	$sql = "select * from tblissue where issue_type='eindent' and issuetrflag=1 and issue_date>='$sdate' and issue_date<='$edate' order by yearcode desc, iss_code desc LIMIT $start, $limit";
	$sql_arr_home = mysql_query($sql);
	if($start==0)$start=1;
	...
	*/
	...
	if($lastpage > 1)
	{	
		$pagination .= "<div class=\"pagination\" style=\"width:805px\" align=\"right\">";
		//previous button
		if ($page > 1) 
			$pagination.= "<a href=\"$targetpage?page=$prev&sdate=$sdate1&edate=$edate1\">« previous</a> ";
		else
		...
		{	
			for ($counter = 1; $counter <= $lastpage; $counter++)
			{
				if ($counter == $page)
					...
					$pagination.= " <a href=\"$targetpage?page=$counter&sdate=$sdate1&edate=$edate1\">$counter</a> ";					
			}
		}
		elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
		{
			...
			if($page < 1 + ($adjacents * 2))		
			{
				...
				$pagination.= "...";
				$pagination.= " <a href=\"$targetpage?page=$lpm1&sdate=$sdate1&edate=$edate1\">$lpm1</a> ";
				$pagination.= " <a href=\"$targetpage?page=$lastpage&sdate=$sdate1&edate=$edate1\">$lastpage</a> ";		
			}
			//in middle; hide some front and some back
			elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
			{
				$pagination.= " <a href=\"$targetpage?page=1&sdate=$sdate1&edate=$edate1\">1</a> ";
				$pagination.= " <a href=\"$targetpage?page=2&sdate=$sdate1&edate=$edate1\">2</a> ";
				$pagination.= "...";
		...
		//next button
		if ($page < $counter - 1) 
			$pagination.= " <a href=\"$targetpage?page=$next&sdate=$sdate1&edate=$edate1\">next »</a> ";
		else
...
<tr class="Light" height="20">
  <td colspan="6" align="center" class="tblheading">Internal Issue-e-Indent </td>
</tr>
</table>
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="13%" align="center" valign="middle" class="tblheading">e-Indents Issue Date</td>
              <td width="17%" align="center" valign="middle" class="tblheading">Indent Number</td>
			  <td width="22%" align="center" valign="middle" class="tblheading">e-Indent Raiser</td>
...
while($row_arr_home=mysql_fetch_array($sql_arr_home))
{
	...
	$sql_ier=mysql_query("select * from tbl_ieindent where  code='".$row_arr_home['dcrefno']."' and yearcode='".$row_arr_home['yearcode']."'") or die(mysql_error());
	$row_ier=mysql_fetch_array($sql_ier);
	$tid=$row_ier['tid'];
	$sql_rol=mysql_query("SELECT * FROM tbl_roles where id='".$row_ier['id']."'")or die(mysql_error()); 
	$row_rol=mysql_fetch_array($sql_rol);
...
<tr class="Light">
         ...
         <td width="12%" align="center" valign="middle" class="tblheading"><a href="add_issue_eindents_view.php?p_id=<?php echo $row_arr_home['issue_id'];?>&tid=<?php echo $tid;?>"><?php echo "IE".$row_arr_home['iss_code']."/".$row_arr_home['yearcode']."/".$irole;?></a></td>
         ...
         <td width="22%" align="center" valign="middle" class="tblheading"><?php echo $row_rol['name']?></td>
         <td width="13%" align="center" valign="middle" class="tblheading"><a href="Javascript:void(0)" onClick="openslocpopprint('<?php echo $row_arr_home['issue_id'];?>','<?php echo $tid;?>');">EIN</a></td>
</tr>
<?php
}
else
...
?>
 <tr  height="25">
 <td align="center" class="subheading" colspan="6">No Records found for selected Period</td>
 </tr>
<?php
}
?> 
...
<tr >
<td valign="top" align="right"><a href="home_transction_issue_einternal.php"><img src="../images/back.gif" border="0"style="display:inline;cursor:pointer;" /></a></td>
</tr>
</table>

...
$sql_tbl=mysql_query("select * from tbl_captive where ccrole='".$logid."' and (ccflg!=1 OR ccflg IS NULL)") or die(mysql_error());
while($row_tbl=mysql_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['tid'];	
	
	$s_sub="delete from tbl_captivesub where id_in='".$arrival_id."'";
	mysql_query($s_sub) or die(mysql_error());
	
	$s_sub_sub="delete from tbl_captive_sloc where issue_trid='".$arrival_id."'";
	mysql_query($s_sub_sub) or die(mysql_error());
	
}

	$s_sub="delete from tbl_captive where ccrole='".$logid."' and (ccflg!=1 OR ccflg IS NULL)";
	mysql_query($s_sub) or die(mysql_error());	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='c_c_home1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='c_c_home.php'</script>";
		}
	}
...
<title>Stores Transaction - Captive Consumption</title>
...
{
winHandle=window.open('cc_issue_note_print.php?itmid='+tid,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
...
{
winHandle=window.open('gatepass_cc.php?itmid='+itm,'WelCome','top=170,left=180,width=820,height=450,scrollbars=yes');
if(winHandle==null){
alert("While Launching New Window...\nYour browser maybe blocking up Popup windows. \n\n  Please check your Popup Blocker Settings or ..\n Please hold Ctrl Key and Click on link to open new Browser"); 
}
}
  ...
  <tr>
    <td valign="top"><table width="1003" height="72" border="0" cellspacing="0" cellpadding="0" align="center">
      ...
      <table width="100%" style=" z-index:-1;" height="auto" align="center" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td width="100%"  height="500" valign="top"align="center"  class="midbgline">
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="820" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="820" height="25">&nbsp;Transaction - Captive Consumption</td>
	    </tr></table></td>
...
<td align="center" width="100" valign="middle" class="tblbutn" style="cursor:hand;"><?php if($flg==0 && $flg1==0 && $flg2==0) { ?><a href="add_internalcc.php" style="text-decoration:none; color:#FFFFFF">Add </a><?php } else { ?><a href="Javascript:void(0);" onclick="alert('Can not perform Transaction.\nReason:\n1. Todays date is not in Active Current Financial Year.\n2. Transaction after todays date has been made.\n3. Cycle Inventory transaction is under process.');" style="text-decoration:none; color:#FFFFFF">Add </a><?php } ?></td>
</tr>
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from tbl_captive where  ccflg =1 order by yearcode desc, cc_code desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from tbl_captive where  ccflg =1 order by yearcode desc, cc_code desc";
...
$sql_arr_home=mysql_query("select * from tbl_captive where  ccflg =1 order by yearcode desc, cc_code desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_captive where ccflg=1"),0); */
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
            ...
            <tr class="tblsubtitle" height="20">
			 ...
			 <td width="16%" align="center" valign="middle" class="tblheading">Transaction Id</td>
			 <td width="14%" align="center" valign="middle" class="tblheading">Date</td>
              <td width="46%" align="center" valign="middle" class="tblheading">Party</td>
...
while($row_arr_home=mysql_fetch_array($sql_arr_home))
{
	...
	$crole=$row_arr_home['ccrole'];
	
if($row_arr_home['party_id'] != "")
	{ 
	$quer3=mysql_query("SELECT * FROM tbl_partymaser  where p_id='".$row_arr_home['party_id']."'"); 
	...
	{
	$business_name=$row_arr_home['party_name'];
...
<tr class="Light">
         ...
         <td width="16%" align="center" valign="middle" class="tblheading"><a href="add_cc_view.php?p_id=<?php echo $row_arr_home['tid'];?>"><?php echo "CC".$row_arr_home['cc_code']."/".$row_arr_home['yearcode']."/".$crole;?></a></td>
		 <td width="14%" align="center" valign="middle" class="tblheading"><?php echo $trdate;?></td>
         <td width="46%" align="center" valign="middle" class="tblheading"><?php echo $business_name;?></td>
         <td width="18%" align="center" valign="middle" class="tblheading"><a href="Javascript:void(0)" onclick="openslocpopprint('<?php echo $row_arr_home['tid'];?>');">CCN</a> | <a href="Javascript:void(0)" onclick="openslocpopprint1('<?php echo $row_arr_home['tid'];?>');">Gate Pass</a></td>
</tr>
<?php
}
else

...
$sql_tbl=mysql_query("select * from tbl_gtod where gdflg=0") or die(mysql_error());
while($row_tbl=mysql_fetch_array($sql_tbl))
{
	$arrival_id=$row_tbl['gid'];	
	
	$s_sub="delete from tbl_gtod_sub where gid='".$arrival_id."'";
	mysql_query($s_sub) or die(mysql_error());
	
}

	$s_sub="delete from tbl_gtod where gdflg=0";
	mysql_query($s_sub) or die(mysql_error());	
	...
	if(isset($_POST['frm_action'])=='submit')
	{
		...
		if($sdate1!="" && $edate1!="")
		{
		echo "<script>window.location='add_g1.php?sdate=$sdate1&edate=$edate1'</script>";
		}
		else
		{
		echo "<script>window.location='add_g.php'</script>";
		}
	}
...
<title>Stores- Transaction - G TO D- Home</title>
  ...
  <tr><td>
   <table  width="974" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" >
	  ...
	  <table width="809" border="0" cellpadding="0" cellspacing="0" bordercolor="#b9d647" style="border-bottom:solid; border-bottom-color:#4ea1e1" >
	    <tr >
	      <td width="810" height="25">&nbsp;Transaction - Good to Damage</td>
	    </tr></table></td>
...
<table align="center" border="1" cellspacing="0" cellpadding="0" width="700" bordercolor="#4ea1e1" style="border-collapse:collapse">
  ...
  <td width="155" class="tblheading" align="center"><input type="image" src="../images/search.gif" border="0" onclick="return mySubmit();"  /></td>
  </tr>
  </table><br />
...
$targetpage = $PHP_SELF; 
  ...
  $sql_arr_home=mysql_query("select * from  tbl_gtod where  gdflg=1 order by yearcode desc, gcode desc LIMIT $start, $limit") or die(mysql_error());
...
$query = "select * from  tbl_gtod where  gdflg=1 order by yearcode desc, gcode desc";
...
$sql_arr_home=mysql_query("select * from  tbl_gtod where  gdflg=1 order by yearcode desc, gcode desc LIMIT $from, $max_results") or die(mysql_error());
$tot_arr_home=mysql_num_rows($sql_arr_home);*/

$total_results = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM tbl_gtod where gdflg=1"),0); 

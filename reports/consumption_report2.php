...
if($_GET['txtclass'] != 'ALL')
{
	 $sql2 = "select DISTINCT(stlg_trclassid) from tbl_stldg_good where stlg_trdate<='$edate' and stlg_trdate>='$sdate' and stlg_trclassid=".$cid." group by stlg_trdate order by stlg_trdate asc";
}	else {
	$sql2 = "select DISTINCT(stlg_trclassid) from tbl_stldg_good where stlg_trdate<='$edate' and stlg_trdate>='$sdate'  group by stlg_trdate order by stlg_trdate asc";
}	
	$rs231 = mysql_query($sql2) or die(mysql_error());	 
	
	while($row231 = mysql_fetch_array($rs231))
	{
	
if($_GET['txtitem']!="ALL")
	$sql = "select DISTINCT(stlg_tritemid),stlg_trclassid from tbl_stldg_good where stlg_trdate<='$edate' and stlg_trdate>='$sdate' and stlg_trclassid=".$row231['stlg_trclassid']." and stlg_tritemid=".$itemid." group by stlg_trdate order by stlg_trdate asc";
	else
	$sql = "select DISTINCT(stlg_tritemid),stlg_trclassid from tbl_stldg_good where stlg_trdate<='$edate' and stlg_trdate>='$sdate' and stlg_trclassid=".$row231['stlg_trclassid']." group by stlg_trdate order by stlg_trdate asc";
	$rs23 = mysql_query($sql) or die(mysql_error());	 
	
	while($row23 = mysql_fetch_array($rs23))
	{
		$itemid = $row23['stlg_tritemid'];
	$sql = "select * from tbl_stldg_good where stlg_trdate<='$edate' and stlg_trdate>='$sdate' and stlg_trclassid=".$row231['stlg_trclassid']." and stlg_tritemid=".$itemid." group by stlg_trdate order by stlg_trdate asc";
	$rs = mysql_query($sql) or die(mysql_error());	  
...
if($_GET['txtclass'] != 'ALL')
{
	...
	{
	if($_GET['txtitem']!="ALL")
	$sql = "select DISTINCT(stld_tritemid),stld_trclassid from tbl_stldg_damage where  stld_trdate <= '$edate' and stld_trdate >= '$sdate' and stld_trclassid =".$row231['stlg_trclassid']."  and stld_tritemid =".$itemid." group by stld_trdate order by stld_trdate ASC";
	else
	$sql = "select DISTINCT(stld_tritemid),stld_trclassid from tbl_stldg_damage where  stld_trdate <= '$edate' and stld_trdate >= '$sdate' and stld_trclassid =".$row231['stlg_trclassid']."   group by stld_trdate order by stld_trdate ASC";
	$rs23 = mysql_query($sql) or die(mysql_error());
	
	while($row23 = mysql_fetch_array($rs23))
	{
		$itemid = $row23['stlg_tritemid'];
	$sql = "select * from tbl_stldg_damage where  stld_trdate <= '$edate' and stld_trdate >= '$sdate' and stld_trclassid =".$row231['stlg_trclassid']."  and stld_tritemid =".$itemid." group by stld_trdate order by stld_trdate ASC";
	$rs = mysql_query($sql) or die(mysql_error());	  
...
<table align="center" width="750" border="0" cellpadding="0" cellspacing="0" bordercolor="#ffffff" style="border-collapse:collapse">
<tr >
<td align="right">&nbsp;<a href="excel-excel_consumption_report.php?pid=<?php echo $_GET['pid'];?>&sdate=<?php echo $_REQUEST['sdate'];?>&txtclass=<?php echo $_REQUEST['txtclass'];?>&txtitem=<?php echo $_REQUEST['txtitem'];?>&ret=<?php echo $_REQUEST['ret'];?>&edate=<?php echo $_REQUEST['edate'];?>" target="_blank"><img src="../images/excelicon1.jpg" border="0" height="30" width="30" class="butn" alt="Export to Excel" style="display:inline;cursor:hand;" /></a>&nbsp;&nbsp;<img src="../images/Vista-printer.png" border="0" class="butn" height="29" width="35" alt="Print" style="display:inline;cursor:hand;" onClick="javascript:window.print();" />&nbsp;&nbsp;&nbsp;<!--<a href="word_bin.php?whid=<?php echo $whid?>"><img src="../images/mswordicon.jpg" border="0" class="butn" height="30" width="30" alt="Export to MS-Word" /></a>&nbsp;&nbsp;&nbsp;--><img src="../images/close_icon2.jpg" border="0" class="butn" height="30"  alt="Close" style="display:inline;cursor:hand;" onClick="window.close()" /></td>
</tr>
</table>

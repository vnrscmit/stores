...
<?php 
$srno=1;
$snn22=mysql_query("select distinct stlg_subbinid, stlg_whid, stlg_binid from tbl_stldg_good where stlg_trclassid='".$cid."' and stlg_tritemid='".$itemid."' group by stlg_trid") or die(mysql_error());
$tot_m22=mysql_num_rows($snn22);
$cnt2=0;
$sbid3 = 0;
while($row22 = mysql_fetch_assoc($snn22))
{

$cnt2++;

if($sbid3!="") 
$sbid3=$sbid3.",".$row22['stlg_subbinid'];
else
$sbid3=$row22['stlg_subbinid'];
}
//echo $sbid3;
...
//echo $ss= "select distinct stlg_subbinid, stlg_whid, stlg_binid from tbl_stldg_good where stlg_trclassid='".$row['stlg_trclassid']."' and stlg_tritemid='".$row['stlg_tritemid']."'  ";
//echo $row['stlg_tritemid']."<br>";





$snn=mysql_query("select distinct stlg_whid, stlg_subbinid, stlg_binid from tbl_stldg_good where stlg_trclassid='".$row['stlg_trclassid']."' and stlg_tritemid='".$row['stlg_tritemid']."' and stlg_trdate <= '$edate' and stlg_trdate >= '$sdate' group by stlg_trid") or die(mysql_error());
...
{	
	...
	{ 
		echo "$dt == fid=".$fid."==";
		//echo "Mukesh";
		$sql_new12345=mysql_query("select * from tbl_stldg_good where stlg_trdate='$dt' and stlg_tritemid ='$itemid' and stlg_subbinid ='$fid' and stlg_balqty >= 0 group by stlg_trid order by stlg_id asc") or die(mysql_error());
echo $tt22=mysql_num_rows($sql_new12345);		
while($ttt45=mysql_fetch_array($sql_new12345))
{
echo "  --  ".$ttt45['stlg_trid']." -> ";
//}

	//echo " <br>  ";
	$sql_new123=mysql_query("select * from tbl_stldg_good where stlg_trdate='$dt' and stlg_tritemid ='$itemid' and stlg_subbinid ='$fid' and stlg_trid='".$ttt45['stlg_trid']."' and stlg_balqty >= 0 group by stlg_trid order by stlg_id asc") or die(mysql_error());
$ttt=mysql_num_rows($sql_new123); echo $ttt." <br>  ";
if($ttt == 0)
{
...
$issups = ""; 
$issqty = "";
$perticulars="SLOC Updation - Updated";
$cn=0;
}
...
$recups = ""; 
$recqty = "";
$perticulars="SLOC Updation - Original Location";
$cn=0;
}*/

...
/**
 * GET availability rows for one indent line (legacy
 * getuser_issue_eindent_etd.php): latest positive balance per SLOC for the
 * item, so the distribution form can re-render without a full page load.
 */
class EIssueLineController extends Controller
{
    public function availability(EIndent $indent, int $line): JsonResponse
    {
        $this->authorize('post-transactions');

        $lineModel = $indent->items()->where('eid', $line)->firstOrFail();

        $locations = StockLedgerGood::query()
            ->select('stlg_whid', 'stlg_binid', 'stlg_subbinid')
            ->where('stlg_tritemid', $lineModel->items_id)
            ->groupBy('stlg_whid', 'stlg_binid', 'stlg_subbinid')
            ->get()
            ->map(function ($loc) use ($lineModel) {

...
<?php $srno=1;
$sql_tbl=mysql_query("select distinct stlg_subbinid, stlg_whid, stlg_binid from tbl_stldg_good where stlg_whid='".$wid."' and stlg_binid='".$bid."' and stlg_trsubtype='$tp'") or die(mysql_error());  
//echo $t=mysql_num_rows($sql_tbl);
while($row_tbl=mysql_fetch_array($sql_tbl))
{

$sql_tbl1=mysql_query("select max(stlg_id) from tbl_stldg_good where stlg_whid='".$wid."' and stlg_binid='".$bid."' and stlg_subbinid='".$row_tbl['stlg_subbinid']."'") or die(mysql_error());  
$row_tbl1=mysql_fetch_array($sql_tbl1);
//echo $t1=mysql_num_rows($sql_tbl1);

$sql1=mysql_query("select * from tbl_stldg_good where stlg_id='".$row_tbl1[0]."'")or die(mysql_error());

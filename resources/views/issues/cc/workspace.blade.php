...
@section('title', 'Captive Consumption — workspace')

@section('content')
<div class="card" x-data="ccWorkspace()">
    <h2>Captive Consumption — TCC{{ $captive->code }}/{{ $captive->yearcode }}</h2>

    <table class="data" style="margin-bottom:1rem">
        <tr>
            <th>Transaction</th>
            <td><strong>TCC{{ $captive->code }}/{{ $captive->yearcode }}</strong></td>
            <th>Date</th>
            <td>{{ optional($captive->tdate)->format('d-m-Y') }}</td>
        </tr>
        <tr>
            <th>Party</th>
            <td>{{ $captive->party_name ?: ($captive->party_id ?: '—') }}</td>
            <th>Return Type</th>
        ...
        <tr>
            ...
            <th>Stage</th>
            <td><span class="badge">{{ (int) $captive->ccflg === 1 ? 'Posted (stock updated)' : 'Open (awaiting final post)' }}</span></td>
        </tr>
        @if ($captive->remarks)
    ...
    @forelse ($lines as $state)
        ...
        <div style="border:1px solid var(--line); border-radius:.5rem; padding:.8rem; margin-bottom:.8rem">
            <div style="display:flex; gap:1rem; align-items:center; flex-wrap:wrap">
                ...
                <span class="muted">Condition {{ $line->type }}</span>
                <span class="muted">Qty {{ $line->qty }}</span>
            </div>
                            ...
                            <td>
                                @if (! $captive->isPosted())
                                    <button type="button" class="btn secondary"
                                            @click="removeLine({{ $line->eid }})">Remove line</button>
                                @endif
                            </td>
                        </tr>
                    @endforeach
    ...
    @if (! $captive->isPosted())
        ...
        <div class="filters" style="display:grid; grid-template-columns:repeat(auto-fit,minmax(12rem,1fr)); gap:1rem">
            ...
            <div><label>Item condition *</label>
                <select x-model="line.type"><option value="good">Good</option></select>
            </div>
        ...
        <form method="POST" action="{{ route('issues.cc.post', $captive) }}"
              onsubmit="return confirm('Post this transaction? Stock will be updated.')"
              style="margin-top:1.2rem">
            @csrf
            <button class="btn" type="submit">Final post (update stock)</button>
        </form>
    @else
        <p class="muted">Posted — committed as CC{{ $captive->cc_code }}. Rows are immutable.</p>
    @endif
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('ccWorkspace', () => ({
            ...
            async removeLine(lineId) {
                ...
                const resp = await fetch(`{{ route('issues.cc.lines.delete', ':line') }}`.replace(':line', lineId), {
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content },
                });
